...
f = open(save_results_path + "MahalanobisFree_results_correct_.csv", "w", encoding="UTF8")
writer = csv.writer(f)
...
for i in range(len(all_models)):
    ...
    for j in range(len(all_id_datasets)):
        ...
        for k in range(len(all_ood_datasets[j])):
            ...
            for lay in range(2):
                monitor = MahalanobisMonitor(id_dataset, model, lay, is_tied=False)
                monitor.fit(features_train[lay][pred_train == lab_train],
                            pred_train[pred_train == lab_train], save=False)
                # monitor.fit(features_train[lay], lab_train, save=False)

                # monitor = OutsideTheBoxMonitor(n_clusters=1)
                # monitor.fit(features_train[lay][pred_train == lab_train],
                #             pred_train[pred_train == lab_train])
                # monitor.fit(features_train[lay], lab_train)
